# Content: Cox regression for each Olink feature
...
for (event in allEvents) {
  result <- pbmclapply(olinkFeatures,
    ...
    function(col) {
      ...
      df <- df %>% left_join(clinical %>% dplyr::select(eid, all_of(clinicalFeatures)),
        by = "eid"
      )
...
saveRDS(allResult, str_glue("Result/coxReg_clinical.rds"))
